...
function getSafePath(relativePath: string, workspaceId?: string) {
  const workspaceRoot = workspaceId ? path.join(process.cwd(), 'workspaces', workspaceId) : process.cwd();
...
export async function POST(req: Request) {
  try {
    ...
    if (process.env.GROQ_API_KEY) {
      ...
      model = groq('llama-3.3-70b-versatile'); // Use latest 70b, 8b hallucinates tool syntax on errors
          ...
          try {
            if (content.toolName === 'writeFile') {
              ...
              if (process.env.AGENT_DEV_LOCAL_EXECUTION !== 'true') {
                throw new Error("Local execution is disabled. Production Docker execution not yet implemented.");
              }
              const workspaceRoot = workspaceId ? path.join(process.cwd(), 'workspaces', workspaceId) : process.cwd();
              const { stdout, stderr } = await execPromise(content.args.command, { cwd: workspaceRoot, timeout: 15000 });
              content.result = `Exit Code 0\nSTDOUT:\n${stdout}\nSTDERR:\n${stderr}`;
              logAgentAction('runCommand', { command: content.args.command, status: 'success', stdout, stderr });
            }
          } catch (err: any) {
            content.result = `Error executing tool: ${err.message}`;
            logAgentAction(content.toolName, { args: content.args, status: 'error', error: err.message });
          }
        }
      }
    }
...
4. DO NOT read more than 1 or 2 files at once. Reading too many files simultaneously will crash the system due to token limits. If the user asks you to read all files, politely decline and ask them which specific file to start with.`,
      ...
      tools: {
        ...
        runCommand: tool({
          description: 'Run a shell command in the workspace terminal. REQUIRES HUMAN APPROVAL.',
          parameters: z.object({
